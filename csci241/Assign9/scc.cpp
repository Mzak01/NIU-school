...
ps: sorry about wonky spacing its half from me folling the psudeocode
    other half is me being tired :)
***************************************************************************/
#include <iostream>
#include <iomanip>
#include <cstdlib>
...
Arguments: buffer1
	   ...
	   post_location
	   var_location
	   (the other things from the private area)
...
void scc::first_pass()
{
    ...
    while (getline(cin, buffer1))
    {
        ...
        //checks for room in the mem array to add instruction
	   ...
	   //symbol table yes
           ...
           //symb not found allocate a new slot
           if (found == false)
           {
        ...
        else if (command == "let")
        {
                ...
                //spot for variable in memory and add to symbol table
                if (found == false)
                {
                        memory[next_const_or_var_addr] = 0;
                ...
                //Go through buffer to find equal sign
                for (j = 0; j < buffer2.length(); j++)
                {
                        if (buffer2[j] == '=')
                        {
                                ...
                                {
                                        infix += buffer2[k]; //Append element after equal sign to infix
                                }
                                break;
                        }
                }
...
Use: The purpose of the second pass is to complete
     the partial instructions written in the first pass.
...
{
        int j = 0;
        int lineloc; //Line location of stack
        int index = 0;
		...
		//get refrences and do instruction
                if (flags[i] > 0)
                {
                        while (j < next_symbol_table_idx)
                        {
                                ...
                                else if (lineloc < next_instruction_addr)
    				{
                ...
                {
                        index = -3 - flags[i];
                        memory[i] += next_const_or_var_addr - index;
                }
        }
}
...
Use: prints the array of memory in the program

return: nothing cause why not at this point
**************************************************************/
